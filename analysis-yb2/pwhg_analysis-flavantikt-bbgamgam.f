...
c  You can substitute these  with your favourite ones
     ...
     f     nbins_pT_bjet2_2=6,nbins_dr_bb_2=8,nbins_dr_Hbb_2=9,nbins_A_Hbb_2=9,
     f     nbins_m_bb_2=10,nbins_m_Hbb_2=8,nbins_m_2b2g=31
...
c      data xx_m_2b2g /150d0, 200d0, 250d0, 300d0,350d0,400d0,450d0,500d0,550d0,600d0,650d0,700d0,750d0,800d0,850d0,900d0/
      data xx_m_2b2g /100d0, 140d0, 180d0,220d0,240d0,260d0,280d0,300d0,320d0,340d0,360d0,380d0,400d0,420d0,440d0
     f       ,460d0,480d0,500d0,520d0,540d0,560d0,580d0,600d0,620d0,640d0,660d0,680d0,700d0,750d0,800d0,850d0,900d0/
      integer icut, iicut
...
c
      call bookupeqbins('yphoton1',dy,-8d0,8d0)
      call bookupeqbins('yphoton2',dy,-8d0,8d0)
c
      call bookupeqbins('mass_gamgam',dmm,0d0,200d0)
c
      call bookupeqbins('pt_gamgam',dpt,ptmin,ptmax)
      call bookupeqbins('ptzoom_gamgam',dpt*0.1d0,ptmin,ptmax*0.1d0)
c
c      call bookupeqbins('ptphoton1',dpt,0d0,500d0)
c      call bookupeqbins('ptphoton1zoom',dptzoom,0d0,50d0)
c      call bookupeqbins('ptphoton2',dpt,0d0,500d0)
c      call bookupeqbins('ptphoton2zoom',dptzoom,0d0,50d0)

c Fid setup:

c  Photon exp cuts (ATLAS/OTHER)----------------------------------------------------------------------------------------

c      do iicut=1,2
c         call bookupeqbins('total'//trim(suffix_exp(iicut)),1d0,0d0,1d0)
c
c         call bookupeqbins('y_Higgs'//trim(suffix_exp(iicut)),dy,-8d0,8d0)
c         call bookupeqbins('eta_Higgs'//trim(suffix_exp(iicut)),dy,-8d0,8d0)
c
c         call bookupeqbins('pt_Higgs'//trim(suffix_exp(iicut)),dpt,0d0,500d0)
c         call bookupeqbins('ptHzoom'//trim(suffix_exp(iicut)),dptzoom,0d0,50d0)
c
c         call bookupeqbins('mass_gamgam'//trim(suffix_exp(iicut)),dmm,0d0,200d0)
c         call bookupeqbins('pt_gamgam'//trim(suffix_exp(iicut)),dpt,0d0,500d0)
c         call bookupeqbins('ptzoom_gamgam'//trim(suffix_exp(iicut)),dpt,0d0,50d0)
c
c         call bookupeqbins('etaphoton1'//trim(suffix_exp(iicut)),dy,-8d0,8d0)
c         call bookupeqbins('etaphoton2'//trim(suffix_exp(iicut)),dy,-8d0,8d0)
c
c         call bookupeqbins('ptphoton1'//trim(suffix_exp(iicut)),dpt,0d0,500d0)
c         call bookupeqbins('ptphoton1zoom'//trim(suffix_exp(iicut)),dptzoom,0d0,50d0)
c         call bookupeqbins('ptphoton2'//trim(suffix_exp(iicut)),dpt,0d0,500d0)
c         call bookupeqbins('ptphoton2zoom'//trim(suffix_exp(iicut)),dptzoom,0d0,50d0)
c
c      enddo
...
c-------------------------------------------------------------------------------
c with photons      
      call bookup('mass_2b2gam-EXP-fid',nbins_m_2b2g,xx_m_2b2g)                  
      call bookupeqbins('m_bb-EXP-fid',dmm,0d0,200d0)

      call bookupeqbins('pt_Higgs-EXP-fid',dpt,ptmin,ptmax)
      call bookup('pt_bjet1_2-EXP-fid',nbins_pT_bjet1_2,xx_pt_bjet1_2)      
      call bookup('pt_bjet2-EXP-fid',nbins_pT_bjet2_2,xx_pt_bjet2_2)

      call bookupeqbins('n_light_jets-EXP-fid',1d0,-0.5d0,6.5d0)

c Fid with cuts for Table 2 in 2307.09992 paper

      call bookupeqbins('total-fid-EXP',1d0,0d0,1d0)
      call bookupeqbins('total-fid-m2b2g-500-EXP',1d0,0d0,1d0)
      call bookupeqbins('total-fid-m2b2g-350-EXP',1d0,0d0,1d0)
c-------------------------------------------------------------------------
...
c added by AS
      call bookup('pt_bjet1_2-EXP'//trim(suffix_CMS(2)),nbins_pT_bjet1_2,xx_pt_bjet1_2)      
...
c Photon exp cuts (ATLAS/OTHER)
c      do iicut=1,2
c      ! distributions 1-bjet
c      call bookup('pt_H-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_pT_H,xx_pt_H)
c      call bookup('pt_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_pT_bjet1,xx_pt_bjet1)
c      call bookupeqbins('y_H-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),dy,-5d0,5d0)
c      call bookup('eta_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_eta_bjet1,xx_eta_bjet1)
cc      call bookup('dphi_H_bjet1-EXP'//trim(suffix_CMS(1)),nbins_dphi_H_bjet1,xx_dphi_H_bjet1)
c      call bookup('dy_H_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_dy_H_bjet1,xx_dy_H_bjet1)
c      call bookup('dR_H_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_dr_H_bjet1,xx_dr_H_bjet1)
c      ! distributions 2-bjet
c      call bookup('pt_H-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_pT_H_2,xx_pt_H_2)
c      call bookupeqbins('y_H-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),dy,-5d0,5d0)
c      call bookup('eta_bjet1-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_eta_bjet1_2,xx_eta_bjet1_2)
c      call bookup('pt_bjet1_2-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_pT_bjet1_2,xx_pt_bjet1_2)      
c      call bookup('pt_bjet2-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_pT_bjet2_2,xx_pt_bjet2_2)
c      call bookup('dR_bb-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_dr_bb_2,xx_dr_bb_2)
c      call bookup('dR_Hbb-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_dr_Hbb_2,xx_dr_Hbb_2)
c      call bookup('m_bb-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_m_bb_2,xx_m_bb_2)
c      enddo
...
c with photons      
      ...
      call bookup('mass_2b2gam-IFN-fid',nbins_m_2b2g,xx_m_2b2g)                  
      call bookupeqbins('m_bb-IFN-fid',dmm,0d0,200d0)

      call bookupeqbins('pt_Higgs-IFN-fid',dpt,ptmin,ptmax)
      call bookup('pt_bjet1_2-IFN-fid',nbins_pT_bjet1_2,xx_pt_bjet1_2)      
      call bookup('pt_bjet2-IFN-fid',nbins_pT_bjet2_2,xx_pt_bjet2_2)

      call bookupeqbins('n_light_jets-IFN-fid',1d0,-0.5d0,6.5d0)
c Fid with cuts for Table 2 in 2307.09992 paper

       call bookupeqbins('total-fid-IFN',1d0,0d0,1d0)
       call bookupeqbins('total-fid-m2b2g-500-IFN',1d0,0d0,1d0)
       call bookupeqbins('total-fid-m2b2g-350-IFN',1d0,0d0,1d0)
c-------------------------------------------------------------------------
      ! distributions 1-bjet
...
c      call bookup('dphi_H_bjet1-IFN'//trim(suffix_CMS(1)),nbins_dphi_H_bjet1,xx_dphi_H_bjet1)
      ...
      call bookup('pt_bjet1_2-IFN'//trim(suffix_CMS(2)),nbins_pT_bjet1_2,xx_pt_bjet1_2)      
...
c      do iicut=1,2
c      ! distributions 1-bjet
c      call bookup('pt_H-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_pT_H,xx_pt_H)
c      call bookup('pt_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_pT_bjet1,xx_pt_bjet1)
c      call bookupeqbins('y_H-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),dy,-5d0,5d0)
c      call bookup('eta_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_eta_bjet1,xx_eta_bjet1)
cc      call bookup('dphi_H_bjet1-EXP'//trim(suffix_CMS(1)),nbins_dphi_H_bjet1,xx_dphi_H_bjet1)
c      call bookup('dy_H_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_dy_H_bjet1,xx_dy_H_bjet1)
c      call bookup('dR_H_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)),nbins_dr_H_bjet1,xx_dr_H_bjet1)
c      ! distributions 2-bjet
c      call bookup('pt_H-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_pT_H_2,xx_pt_H_2)
c      call bookupeqbins('y_H-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),dy,-5d0,5d0)
c      call bookup('eta_bjet1-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_eta_bjet1_2,xx_eta_bjet1_2)
c      call bookup('pt_bjet1_2-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_pT_bjet1_2,xx_pt_bjet1_2)      
c      call bookup('pt_bjet2-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_pT_bjet2_2,xx_pt_bjet2_2)
c      call bookup('dR_bb-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_dr_bb_2,xx_dr_bb_2)
c      call bookup('dR_Hbb-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_dr_Hbb_2,xx_dr_Hbb_2)
c      call bookup('m_bb-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),nbins_m_bb_2,xx_m_bb_2)
c      enddo
c
...
c 
      ...
      character * 10 suffix_CMS(ncuts_CMS),suffix_exp(2)
      common/jcut/jetcut,mbbHcut,suffix,suffix_CMS,suffix_exp
...
c     arrays to reconstruct jets
     ...
     f     ,dphihj1,drhj1,dyhj1,m_b1b2g1g2,m_2b2g
     f     ,y_b1b2g1g2, eta_b1b2g1g2, pt_b1b2g1g2
      integer total_b_jets,counter_b_jets,counter_light_jets,counter_all_jets
      real *8 p_b1b2(4), p_b1b2h(4), p_b1b2g1g2(4)
...
c         do ihep=1,nhep
c
c            if(idhep(ihep).eq.25) then                
c               if(isthep(ihep).eq.1) then !'stable' Higgs
c                  ihiggs=ihep
c                  iihiggs=iihiggs+1
...
c               endif
c            endif !idhep
c            if (idhep(ihep).eq.5) then !bottom
c               ib = ihep
c               iib = iib+1
c            endif   
c            if (idhep(ihep).eq.-5) then !bbar
c               ibbar = ihep
c               iibbar = iibbar+1
c            endif   
c         enddo !nhep
...
c     Check if there is still stable Higgs after PY8 showering
      ...
      if(IIhiggs.eq.0) then     ! no Higgs found, so reconstruct from photons (if found)
         if(n_photon.lt.2) then
            ...
            write(*,*) 'ERROR: Higgs not found and more than two photons found to reconstuct Higgs 
     f                 (needs to be adapted when QED shower or MPI on)'
            call exit(1)
...
c      phiggs(1)=phep(1,ihiggs)
c      phiggs(2)=phep(2,ihiggs)
c      phiggs(3)=phep(3,ihiggs)
c      phiggs(4)=phep(4,ihiggs)
      
c AS: check which one to use 
       call getyetaptmass2(pH,yh,etah,pth,mhiggs)
c       call ptyeta(pH,pth,yh,etah)

c      mhiggs = dsqrt(abs(pH(4)**2-pH(1)**2-
c     &     pH(2)**2-pH(3)**2))

      if(ibottom.ne.0) then
c      call getdydetadphidr(phep(1,ibottom),phep(1,ihiggs),
c     %        ybh,etabh,phibh,rbh)
c      call getdydetadphidrr(phep(1,ibottom),phep(1,ihiggs),
c     %     retabh)


      call getdydetadphidr(phep(1,ibottom),pH,
     %        ybh,etabh,phibh,rbh)
 

      call getdydetadphidrr(phep(1,ibottom),pH,
     %     retabh)


      endif
      if(iabottom.ne.0) then
c      call getdydetadphidr(phep(1,iabottom),phep(1,ihiggs),
c     %     ybbarh,etabbarh,phibbarh,rbbarh)
c      call getdydetadphidrr(phep(1,iabottom),phep(1,ihiggs),
c     %     retabbarh)
 
      call getdydetadphidr(phep(1,iabottom),pH,
     %     ybbarh,etabbarh,phibbarh,rbbarh)
      call getdydetadphidrr(phep(1,iabottom),pH,
     %     retabbarh)
...
c     %     retabbh)
         call getdydetadphidr(ppairbbbar(1),pH,
     %     ybbh,etabbh,phibbh,rbbh)
         call getdydetadphidrr(ppairbbbar(1),pH,
     %     retabbh)
      endif
...
cc y      
      call filld('yphoton1',y_photon1,dsig)
      call filld('yphoton2',y_photon2,dsig)
cc mass
      call filld('mass_gamgam',mgamgam,dsig)
cc pt
      call filld('pt_gamgam',ptgamgam,dsig)
      call filld('ptzoom_gamgam',ptgamgam,dsig)
c
c      call filld('ptphoton1',pt_photon1,dsig)
c      call filld('ptphoton1zoom',pt_photon1,dsig)
c      call filld('ptphoton2',pt_photon2,dsig)
c      call filld('ptphoton2zoom',pt_photon2,dsig)
...
c    OBS for Higgs-j1
c            call getdydetadphidr2(phep(1,ihiggs),p_all_jets(:,ihardest),dyHj1,detaHj1,dphiHj1,drHj1)
            call getdydetadphidr2(pH,p_all_jets(:,ihardest),dyHj1,detaHj1,dphiHj1,drHj1)
...
c      pt_ljet_cut = 30d0
c      eta_ljet_cut = 2.4d0

c AS: cuts used in HH paper
...
c      pt_bjet_cut_CMS = 30d0
c      eta_bjet_cut_CMS = 2.4d0

c AS :Cuts used in HH paper
      pt_bjet_cut_CMS = 25d0
      eta_bjet_cut_CMS = 2.5d0
...
c         call getdydetadphidr2(phiggs,p_b1b2,dy_H_bjet1bjet2,deta_H_bjet1bjet2,dphi_H_bjet1bjet2,dr_H_bjet1bjet2)
         call getdydetadphidr2(pH,p_b1b2,dy_H_bjet1bjet2,deta_H_bjet1bjet2,dphi_H_bjet1bjet2,dr_H_bjet1bjet2)
...
c        bjet1bjet2photon1photon2 momentum (but it is same as above)
         do mu=1,4
            p_b1b2g1g2(mu)=p_b1b2(mu)+pH(mu)
         enddo
         !AS, I need to construct the m*2b2g observable in eq 8 of paper:2307.09992
         call getyetaptmass2(p_b1b2g1g2,y_b1b2g1g2,eta_b1b2g1g2,pt_b1b2g1g2,m_b1b2g1g2)

c         m_b1b2g1g2=dsqrt(abs(p_b1b2g1g2(4)**2
c     1            -p_b1b2g1g2(1)**2-p_b1b2g1g2(2)**2-p_b1b2g1g2(3)**2))

         m_2b2g = m_b1b2g1g2 - m_b1b2 - mgamgam + 2*mhiggs
c         print*,'mass_b1b2g1g2', m_b1b2g1g2
c         print*,'mass_b1b2', m_b1b2
c         print*,'mass_gamgam', mgamgam
c         print*,'mass_higgs', mhiggs
      endif

c------------------------------------------------------------------------------------------------
c Photon fid cuts as implemented in 2307.09992 with 2 b-tagging 

      if(n_bjets .ge. 2) then
        if((abs(eta_photon1) .lt. 2.37) .and. (abs(eta_photon2).lt.2.37)) then
            if( (pt_photon1 .gt. 0.35*mHiggs) .and. (pt_photon2.gt.0.25*mHiggs)) then
             if( (m_b1b2 .lt. 140) .and. (m_b1b2.gt. 80)) then
            call filld('total-fid-EXP',0.5d0, dsig)
            call filld('mass_2b2gam-EXP-fid', m_2b2g, dsig)
            call filld('m_bb-EXP-fid', m_b1b2, dsig)

            call filld('pt_Higgs-EXP-fid',pth,dsig)
            call filld('pt_bjet1_2-EXP-fid',pT_bjet1,dsig)
            call filld('pt_bjet2-EXP-fid',pT_bjet2,dsig)

            call filld('n_light_jets-EXP-fid',1d0*counter_light_jets,dsig)
           endif
          endif
        endif
        ...
        if((abs(eta_photon1) .lt. 2.37) .and. (abs(eta_photon2).lt.2.37)) then
            if( (pt_photon1 .gt. 0.35*mHiggs) .and. (pt_photon2.gt.0.25*mHiggs)) then
             if( (m_b1b2 .lt. 140) .and. (m_b1b2.gt. 80)) then
              if( m_2b2g .lt. 500) then
            call filld('total-fid-m2b2g-500-EXP',0.5d0, dsig)
              endif
              if ( m_2b2g .lt. 350) then
            call filld('total-fid-m2b2g-350-EXP',0.5d0, dsig)
              endif
           endif
          endif
        endif
...
c--------------------------------------------------------------------------------------
      ...
      if(n_bjets .ge. 1) then
         ...
         if(n_bjets .ge. 2) then
            ...
            call filld('pt_bjet1_2-EXP'//trim(suffix_CMS(2)),pT_bjet1,dsig)
...
cc Photon exp cuts  ATLAS/OTHER (independent of jet algorithm) -----------------------------------------
c
c        do iicut=1,2
c         if( (abs(eta_photon1) .le. 1.37 .or. (abs(eta_photon1) .le. 2.37 .and. abs(eta_photon1) .ge. 1.52) )
c     $ .and. (abs(eta_photon2) .le. 1.37 .or. (abs(eta_photon2) .le. 2.37 .and. abs(eta_photon2) .ge. 1.52) ) ) then
c            if( (iicut.eq.1 .and. pt_photon1 .ge. 0.35*mHiggs .and. pt_photon2.ge. 0.25*mHiggs)
c     $     .or. (iicut.eq.2 .and. dsqrt(pt_photon1*pt_photon2) .ge.0.35*mHiggs.and. pt_photon2 .ge. 0.25*mHiggs)) then 
c               call filld('total'//trim(suffix_exp(iicut)),0.5d0,dsig)
c               call filld('y_Higgs'//trim(suffix_exp(iicut)),yH,dsig)
c               call filld('eta_Higgs'//trim(suffix_exp(iicut)),etaH,dsig)
c
c               call filld('mass_gamgam'//trim(suffix_exp(iicut)),mgamgam,dsig)
c               call filld('pt_gamgam'//trim(suffix_exp(iicut)),ptgamgam,dsig)
c               call filld('ptzoom_gamgam'//trim(suffix_exp(iicut)),ptgamgam,dsig)
c
c               call filld('pt_Higgs'//trim(suffix_exp(iicut)),ptH,dsig)
c               call filld('ptHzoom'//trim(suffix_exp(iicut)),ptH,dsig)
c 
c               call filld('ptphoton1'//trim(suffix_exp(iicut)),pt_photon1,dsig)
c               call filld('ptphoton1zoom'//trim(suffix_exp(iicut)),pt_photon1,dsig)
c               call filld('ptphoton2'//trim(suffix_exp(iicut)),pt_photon2,dsig)
c               call filld('ptphoton2zoom'//trim(suffix_exp(iicut)),pt_photon2,dsig)
c               call filld('etaphoton1'//trim(suffix_exp(iicut)),eta_photon1,dsig)
c               call filld('etaphoton2'//trim(suffix_exp(iicut)),eta_photon1,dsig)
c
c            endif
c         endif
c      enddo
c      
cc Photon exp cuts ATLAS/OTHER with 1 b-tagging
c
c        do iicut=1,2
c         if( (abs(eta_photon1) .le. 1.37 .or. (abs(eta_photon1) .le. 2.37 .and. abs(eta_photon1) .ge. 1.52) )
c     $ .and. (abs(eta_photon2) .le. 1.37 .or. (abs(eta_photon2) .le. 2.37 .and. abs(eta_photon2) .ge. 1.52) ) ) then
c            if( (iicut.eq.1 .and. pt_photon1 .ge. 0.35*mHiggs .and. pt_photon2.ge. 0.25*mHiggs)
c     $     .or. (iicut.eq.2 .and. dsqrt(pt_photon1*pt_photon2) .ge.0.35*mHiggs.and. pt_photon2 .ge. 0.25*mHiggs)) then 
c       if(n_bjets .ge. 1) then
c            ! distributions with at least 1-bjet with photon exp cuts
c            call filld('pt_H-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), pth, dsig)
c            call filld('y_H-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), yh, dsig)
c            call filld('pt_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), pt_bjet1, dsig)
c            call filld('eta_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), abs(eta_bjet1), dsig)
c            call filld('dy_H_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), dy_H_bjet1, dsig)
c            call filld('dR_H_bjet1-EXP'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), dr_H_bjet1, dsig)
c         endif
c           endif
c         endif
c      enddo
c      
cc Photon exp cuts ATLAS/OTHER with 2 b-tagging
c
c        do iicut=1,2
c         if( (abs(eta_photon1) .le. 1.37 .or. (abs(eta_photon1) .le. 2.37 .and. abs(eta_photon1) .ge. 1.52) )
c     $ .and. (abs(eta_photon2) .le. 1.37 .or. (abs(eta_photon2) .le. 2.37 .and. abs(eta_photon2) .ge. 1.52) ) ) then
c            if( (iicut.eq.1 .and. pt_photon1 .ge. 0.35*mHiggs .and. pt_photon2.ge. 0.25*mHiggs)
c     $     .or. (iicut.eq.2 .and. dsqrt(pt_photon1*pt_photon2) .ge.0.35*mHiggs.and. pt_photon2 .ge. 0.25*mHiggs)) then 
c         if(n_bjets .ge. 2) then
c            ! distributions with at least 2-bjet
c            call filld('pt_H-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), pth , dsig)
c            call filld('y_H-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), yh, dsig)
c            call filld('eta_bjet1-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), abs(eta_bjet1), dsig)
c            call filld('pt_bjet1_2-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),pT_bjet1,dsig)
c            call filld('pt_bjet2-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), pt_bjet2, dsig)
c            call filld('dR_bb-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), dr_b1_b2, dsig)
c            call filld('dR_Hbb-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),dr_H_bjet1bjet2,dsig)
c            call filld('m_bb-EXP'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), m_b1b2, dsig) 
c         endif
c           endif
c         endif
c      enddo
...
c Photon fid cuts as implemented in 2307.09992 with 2 b-tagging 
      ...
      if(n_bjets .ge. 2) then
        if((abs(eta_photon1) .lt. 2.37) .and. (abs(eta_photon2).lt.2.37)) then
            if( (pt_photon1 .gt. 0.35*mHiggs) .and. (pt_photon2.gt.0.25*mHiggs)) then
             if( (m_b1b2 .lt. 140) .and. (m_b1b2.gt. 80)) then
            call filld('total-fid-IFN',0.5d0, dsig)
            call filld('mass_2b2gam-IFN-fid', m_2b2g, dsig)
            call filld('m_bb-IFN-fid', m_b1b2, dsig)
  
            call filld('pt_Higgs-IFN-fid', pth, dsig)
            call filld('pt_bjet1_2-IFN-fid',pT_bjet1,dsig)
            call filld('pt_bjet2-IFN-fid',pT_bjet2,dsig)
 
            call filld('n_light_jets-IFN-fid',1d0*counter_light_jets,dsig)
           endif
          endif
        endif
        ...
        if((abs(eta_photon1) .lt. 2.37) .and. (abs(eta_photon2).lt.2.37)) then
            if( (pt_photon1 .gt. 0.35*mHiggs) .and. (pt_photon2.gt.0.25*mHiggs)) then
             if( (m_b1b2 .lt. 140) .and. (m_b1b2.gt. 80)) then
              if( m_2b2g .lt. 500) then
             call filld('total-fid-m2b2g-500-IFN',0.5d0, dsig)
              endif
              if ( m_2b2g .lt. 350) then
              call filld('total-fid-m2b2g-350-IFN',0.5d0, dsig)
              endif
           endif
          endif
        endif
...
c--------------------------------------------------------------------------------------
      ...
      if(n_bjets .ge. 1) then
         ...
         if(n_bjets .ge. 2) then
            ...
            call filld('pt_bjet1_2-IFN'//trim(suffix_CMS(2)),pT_bjet1,dsig)
...
c       if(n_bjets .ge. 1) then
c            ! distributions with at least 1-bjet with photon exp cuts
c            call filld('pt_H-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), pth, dsig)
c            call filld('y_H-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), yh, dsig)
c            call filld('pt_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), pt_bjet1, dsig)
c            call filld('eta_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), abs(eta_bjet1), dsig)
c            call filld('dy_H_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), dy_H_bjet1, dsig)
c            call filld('dR_H_bjet1-IFN'//trim(suffix_CMS(1))//trim(suffix_exp(iicut)), dr_H_bjet1, dsig)
c         endif
c           endif
c         endif
c      enddo
...
c         if(n_bjets .ge. 2) then
c            ! distributions with at least 2-bjet
c            call filld('pt_H-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), pth , dsig)
c            call filld('y_H-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), yh, dsig)
c            call filld('eta_bjet1-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), abs(eta_bjet1), dsig)
c            call filld('pt_bjet1_2-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),pT_bjet1,dsig)
c            call filld('pt_bjet2-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), pt_bjet2, dsig)
c            call filld('dR_bb-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), dr_b1_b2, dsig)
c            call filld('dR_Hbb-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)),dr_H_bjet1bjet2,dsig)
c            call filld('m_bb-IFN'//trim(suffix_CMS(2))//trim(suffix_exp(iicut)), m_b1b2, dsig) 
c         endif
c           endif
c         endif
c      enddo
