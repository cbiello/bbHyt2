      ...
      save mcalls,icalls
      integer mcallsrm,icallsrm
      data mcallsrm,icallsrm/0,0/
      save mcallsrm,icallsrm
      integer mcallsreg,icallsreg
      data mcallsreg,icallsreg/0,0/
      save mcallsreg,icallsreg
...
c     generate underlying Born flavour
         ...
         iind = rad_ubornsubp
         flst_currentuborn=rad_ubornsubp
c
...
c     generate remnant n+1 body cross section
         call reset_timer
         call gen_sigremnant(mcallsrm,icallsrm)
         call get_timer(seconds)
...
c     rad_type=2 for remnants
      ...
      elseif(id.eq.'reg') then
         call reset_timer
         call gen_sigregular(mcallsreg,icallsreg)
         call get_timer(seconds)
...
c     add a random azimuthal rotation around beam axis to the Les Hoches momenta
      ...
      if(flg_weightedev) then
         ...
         elseif(id.eq.'remn') then
c     rmn_suppression may depend upon the alr. It checks the flst_currentalr to see which
c     alr we are considering.
            flst_currentalr = rad_alrsubp
...
c     this is set to 1 if there are ub failure in the
c     generation of radiation
      rad_ubfailrad = 0
      if(ini) then
...
c Set it to a value less than 2, to avoid problems later.
     ...
     1              .and.(is_charged(fl1).or.is_charged(fl2)
     2              .or.fl1==22.or.fl2==22)) then
c     For processes with partons (colored)  in the initial state, we never enter here. 
c     Only when a lepton or photon is incoming, the following applies
c$$$                  write(*,*) 'Initial-state not-coloured charged '//
c$$$     $                 'particle'
c$$$                  write(*,*) 'ISR upper bound not yet implemented'
c$$$                  call pwhg_exit(-1)
                  em_rad_on = .true.
               else
                  if (flg_QEDonly) then
...
c     so we ignore them unless t is significantly above ptsqmin
         if(t > 2d0*rad_ptsqmin) then
            write(*,*) ' Error: upper bound lower than actual value',
     1        tmp,tmp1,tmp2,t,rad_ptsqmin
         endif
      endif
      if(rv.gt.tmp) then
         goto 1
...
c that some pdf vanish (typically heavy flavour pdf's)
      ...
      kn_csi=1-x
      kn_azi=2*pi*random()
      ubound=abs(born*pwhg_upperb_rad()*unorm*ufct)
...
c     endif
      ...
      value=abs(sig*kn_jacreal)
      if(value.gt.ubound) then
         call increasecnt(
     #'upper bound failures in generation of radiation')
c     Set flag for ub failures
         rad_ubfailrad = 1
      endif
      rvalue=random()*ubound
      if(rvalue.gt.value) then
         if (em_rad_on) then
            call increasecnt('ISR vetoed electromagnetic radiation')
         else
            call increasecnt('ISR vetoed strong radiation')
         endif
         goto 1
      endif
      end
...
c      print†∑*, 'rad_ptsqmin', rad_ptsqmin
