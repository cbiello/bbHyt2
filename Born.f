...
c     Wrapper subroutine calling the Recola Born.
c     No bottom Yukawa is involved in the yt^2 contribution, hence
c     none of the running-Yukawa machinery of the yb^2 generator
c     (bbH4FS) is needed here.
      use recola_powheg, only: recola_born
      implicit none
      include 'nlegborn.h'
      include 'pwhg_st.h'
...
c Wrapper subroutine to call the Recola code to associate
c a (leading) color structure to an event.
      use recola_powheg, only: recola_borncolour_ifl
      implicit none
      include 'nlegborn.h'
 ...
 1    continue
      ...
      ! call Recola colour flow
      call recola_borncolour_ifl(p,is_fs_flav,color,ifl)
